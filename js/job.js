...
function job() {
    ...
    // 根据 传入coid改变 公司选中项
    // $(".difsd div").each((index, item) => {
    //     console.log("---", $(item).attr('data-coid'), item);
    //     if (coid == $(item).attr('data-coid')) {
    //         $(".difsd div").removeClass("dif").eq(index).addClass("dif")
    //     }
    // })
    if (!params.coid) {
...
coapi.getJobCondition(params.ctmid, function (data) {
    ...
    console.log(coidArr, agencyId, "-----agencyIdagencyId---------------")

    for (j in coidArr) {
        // inner2 += '<option value="' + coid[j].key + '" selected>' + coid[j].value + '</option>'
